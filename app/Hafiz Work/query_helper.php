...
function get_profitAvg($supplier='')
{
   $options=array();
    ...
    if ($supplier) {
    	$sql = "SELECT sum(price) as profit,avg(price) as average FROM `tbl_booking` WHERE date(booked_at)='$today' AND status='1' AND (reference IS NOT NULL AND reference !='') AND reference NOT LIKE 'GL-%' AND reference NOT LIKE 'GL %'";
    }
...
// Email
function send_single_email($to,$subject,$message)
{ 
