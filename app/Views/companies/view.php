    ...
    <div class="content-header row">
      ...
      <div class="content-header-right text-md-end col-md-3 col-12 d-md-block d-none">
        <button type="button" class="btn btn-primary waves-effect waves-float waves-light" data-bs-toggle="modal" data-bs-target="#add-xlarge">
          <i data-feather='plus'></i> Add Company
        </button>
      </div>
    </div>
...
<div class="modal fade text-start" id="add-xlarge" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <form class="form-crud" id="form-crud" action="<?= base_url('vehicles/companies/save'); ?>">
        <?= csrf_field() ?>
        <input type="hidden" name="id" id="id" value="" />

        <div class="modal-header">
          <h4 class="modal-title" id="form-crud-title">Add Company</h4>
...
var table = $('#view-datatable').DataTable({
    processing: true,
    serverSide: true,
    ajax: "<?= base_url('vehicles/companies/get'); ?>",
...
function edit_data(id){
    $.get("<?= base_url('vehicles/companies/get_record'); ?>?id="+id, function(res){
        if(res.status){
            $("#id").val(id);
            $("#name").val(res.data.name);
            $("#status").val(res.data.status);
            $("#form-crud").attr('action', "<?= base_url('vehicles/companies/update'); ?>");
            $("#form-crud-title").text("Edit Company");
...
function delete_data(id) {
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: "<?= base_url('vehicles/companies/delete_record'); ?>",
          type: 'GET',
          dataType: 'json',
          data: {id: id},
...
$("#add-xlarge").on("hidden.bs.modal", function(){
    $("#form-crud")[0].reset();
    $("#id").val('');
    $("#form-crud-title").text("Add Company");
    $("#form-crud").attr("action", "<?= base_url('vehicles/companies/save'); ?>");
});
