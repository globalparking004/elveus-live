...
<div class="modal fade text-start" id="add-xlarge" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <form class="form-crud" id="form-crud" action="<?= base_url('vehicles/save'); ?>">
        <?= csrf_field() ?>
        <input type="hidden" name="id" id="id" value="" />
        <div class="modal-header">
          <h4 class="modal-title" id="form-crud-title">Add Vehicle</h4>
          ...
          <div class="row">
            ...
            <div class="col-md-6 col-12">
              <div class="mb-1">
                ...
                <select class="form-select select2" id="model_id" name="model_id">
            ...
            <div class="col-md-6 col-12">
              <div class="mb-1">
                ...
                <select class="form-select select2" id="color_id" name="color_id">
            ...
            <div class="col-md-6 col-12">
              <div class="mb-1">
                ...
                <input type="text" id="price_per_day" class="form-control" placeholder="Price Per Day" name="price_per_day" />
              </div>
            </div>
            <div class="col-md-6 col-12">
              <div class="mb-1">
                <label class="form-label" for="quantity">Car Quantity</label>
                <input type="text" id="quantity" class="form-control" placeholder="Car Quantity" name="quantity" />
              </div>
            </div>
            <div class="col-md-6 col-12">
...
$('#make_id').on('change', function(){
    ...
    if(make_id){
        $.get("<?= base_url('vehicles/get_models'); ?>?make_id=" + make_id, function(res){
...
$(".form-crud").validate({
    rules: {
        ...
        price_per_day: { required: true, number: true },
    ...
    messages: {
        ...
        price_per_day: { required: "Please enter price", number: "Price must be a number" },
        quantity: { required: "Please enter quantity", number: "Quantity must be a number" }
    },
...
function edit_data(id){
    $.get("<?= base_url('vehicles/get_record'); ?>?id="+id, function(res){
        if(res.status){

            $("#id").val(id);
            // 1: Set Make
            $("#make_id").val(res.data.make_id).trigger('change');
            // 2: Wait until models are loaded
            setTimeout(function(){

                // Ab AJAX se models load ho chuki hain
                $("#model_id").val(res.data.model_id).trigger('change');

            }, 500);  // <- 500ms waiting time to ensure models are loaded

            $("#color_id").val(res.data.color_id).trigger('change');
            $("#price_per_day").val(res.data.price_per_day);
            $("#quantity").val(res.data.quantity);
            $("#status").val(res.data.status);
            $("#form-crud").attr('action', "<?= base_url('vehicles/update'); ?>");
            $("#form-crud-title").text("Edit Vehicle");
...
function delete_data(id) {
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: "<?= base_url('vehicles/delete_record'); ?>", // Vehicles ka URL
          type: 'GET',
          dataType: 'json',
...
$("#add-xlarge").on("hidden.bs.modal", function(){
    ...
    $("#form-crud").attr("action", "<?= base_url('vehicles/save'); ?>");
});
