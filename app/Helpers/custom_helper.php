...
function id_en($id)
{
   ...
   //return $id;
	$enc = \Config\Services::encrypter()->encrypt($id);
...
{  
   ...
   }elseif($AUTH['role_name']=="Operator" || $AUTH['role_name'] == 'SourceBase')
   {
        $has_booking_sub[]=['href'=>"view","feather"=>"circle","text"=>"Bookings"];
        $menu[]=['href'=>"bookings","feather"=>"bookmark","text"=>"Bookings","has_sub"=>$has_booking_sub];

   }elseif($AUTH['role_name'] == 'Capacity')
   {
        $has_sub[]=['href'=>"bookings/capacity","feather"=>"circle","text"=>"Bookings Capacity"];
        $menu[]=['href'=>"reports","feather"=>"file-text","text"=>"Reports","has_sub"=>$has_sub]; 
        ...
        $menu[]=['href'=>"gopakistan","feather"=>"globe","text"=>"Go Pakistan","has_sub"=>[]];
	    ...
	    $has_vehdub[]=['href'=>"","feather"=>"circle","text"=>"Vehicles"];
        $has_vehdub[]=['href'=>"companies","feather"=>"circle","text"=>"Companies"];
        $has_vehdub[]=['href'=>"companymodels","feather"=>"circle","text"=>"Model"];
        $has_vehdub[]=['href'=>"colors","feather"=>"circle","text"=>"Colors"];
        // $menu[]=['href'=>"vehicles","feather"=>"truck","text"=>"Vehicles Manage","has_sub"=>$has_vehdub];

        // $menu[]=['href'=>"security","feather"=>"user","text"=>"Security Guard","has_sub"=>[]];
    }
...
function get_agents()
{
   $options = [
       ...
       'Cash Booking' => 'Cash Booking',
	    'goairportparking.com' => 'Go Comparison',
        'www.ca.vu' => 'www.ca.vu',
       'Unknown' => 'Unknown',
...
function mergeGoAirportData($array1, $array2) 
{
    $merged = [];
    ...
    foreach ($array1 as $item) {
        if (isset($merged[$item->airport])) {
            $merged[$item->airport]['totalQTY'] = $item->totalQTY;
            $merged[$item->airport]['totAmount'] = $item->totAmount;
        } else {
            $merged[$item->airport] = [
                'airport' => $item->airport,
                'totalQTY' => 0,
...
function get_airport_terminals($terminals=NULL)
{
    $options=array(
    ...
    $html='';
    foreach ($options as $key => $v) {
        // Check if $terminals is an array and contains the current key
        $selected = (is_array($terminals) && in_array($key, $terminals)) ? ' selected="selected"' : '';
...
function get_sources($souces=NULL)
{
    $options=array(
        'SEO'=> 'SEO',
        'Backlink'=> 'Backlink',
    ...
    foreach ($options as $key => $v) {
        // Check if $terminals is an array and contains the current key
        $selected = (is_array($souces) && in_array($key, $souces)) ? ' selected="selected"' : '';
        $html.='<option value="'.$key.'" '.$selected.'>'.$v.'</option>';
    }
