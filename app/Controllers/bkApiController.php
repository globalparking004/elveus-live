        ...
        if (isset($_GET['access_token']) && $_GET['access_token'] == '5MEsB9lLwVqu4qndXvEUE428bqGZY') {
            ...
            if (!$this->validation->withRequest($this->request)->run()) {
                ...
                $sql_data = "INSERT INTO tbl_booking 
                        ...
                        firstName, surname, email, contactNumber ,product_id,airport,booked_at,depart_at,source,return_at,status,operator_id,promocode,promo_price,InFltNo,OutFltNo, passenger, show_status) 
                        VALUES 
                        ('$new_price', '$new_reference', '$carReg', '$carMake', '$carModel', '$carColour', '$required_OutTerminal', '$required_RetTerminal', '$agent',
                        '$firstName', '$surname', '$email', '$contactNumber',$id,'$airport','$formattedDate','$arrival_date','$website','$departure_date','0','$operatorid','$promoCode','$promo_price','$Departure_Flight_Number','$Return_Flight_Number', '$passenger', '1');
                        ";
    ...
    public function create_booking2()
    {
        ...
        if (isset($_GET['access_token']) && $_GET['access_token'] == '5MEsB9lLwVqu4qndXvEUE428bqGZY') {
            ...
            foreach ($result as $r) {
                ...
                if (($r->capacity) != 0) {
                    $cal_capacity = "SELECT count(*) as count FROM `tbl_booking` WHERE  `product_id`= $r->id and (`depart_at`<= '$formated_arrive_date 00:00:00' AND return_at>'$formated_arrive_date 00:00:00')  and status='completed'";
                    ...
                    if (isset($result->price) && !empty($result->price)) {
                        ...
                        if (($r->capacity) != 0) {
                            if (!empty($adjust_prices_by_capacity) && $adjust_prices_by_capacity == $adjust_prices_by__product_capacity) {
                                ...
                                $sql_data = "SELECT count(*) as capacity_full FROM `tbl_booking` WHERE `product_id`= $r->id and (`depart_at`<= '$formated_arrive_date 00:00:00' AND return_at>'$formated_arrive_date 00:00:00')  and status='completed'";
                                $result = $this->db->query($sql_data)->getRow();
                        ...
                        if (!empty($adjust_prices_by_capacity) && $adjust_prices_by_capacity == $adjust_prices_by__operator_capacity) {
                            ...
                            $sql_data = "SELECT count(*) as operator_capacitys  FROM `tbl_booking` WHERE `operator_id`= $r->operator_id and (`depart_at`<= '$formated_arrive_date 00:00:00' AND return_at>'$formated_arrive_date 00:00:00')  and status='completed'";
                            $result = $this->db->query($sql_data)->getRow();
