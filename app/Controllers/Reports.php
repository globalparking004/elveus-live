...
{   
    ...
    public function get_aff_performance()
    {
        ...
        $tfooter = array("Total","",$bookings,round($amount,2), round($bookingFee,2));
        if($data):
            array_push($data, $tfooter);
        endif;
    ...
    public function get_go_aff_performance()
    {
        $data = $this->request->getVar();
        $search = $this->request->getVar('search')['value'];
        ...
        $SQLFilterDate = "";
        $SQLWebsiteCondition = " AND source IS NOT NULL AND (source ='Dashboard' OR source NOT LIKE '%Dashboard') AND source='goairportparking.com' AND source !='CPD' AND source!='CTAP' AND source !='P4U' AND source!='Holiday Extras' AND source!='ParkVia' AND source!='Park&Fly' AND source!='FreeToMove' AND source!='Airport Parking With Us' AND source!='JBF' AND source!= 'Cash Booking' AND source!='YTE' AND source!='HCP' AND source!='CYP' AND source !='https://longtermparking.ie/' AND source !='Go Comparison' AND source !='www.ca.vu' AND source !='skyparkingservices.co.uk' ";
    ...
    public function get_performance_supplier()
    {
        ...
        if (empty($Airport)) {
            ...
            $result2 = $this->mergeAndSumByName($result,'www.ca.vu');
            $result2 = $this->mergeAndSumByName($result,'skyparkingservices.co.uk');
        }
        // pre($result2);
    ...
    public function get_go_performance()
    {
        $data = $this->request->getVar();
        $search = $this->request->getVar('search')['value'];
    ...
    public function get_departure_return()
    {
        ...
        foreach ($dates as $key => $d) 
        {
            ...
            $sql_dataR = "SELECT count(*) as totReturns,SUM(price) as totPrice FROM `tbl_booking`  WHERE 1=1 $SQLReturnDate $SQLstatus $SQLairport $SQLoperator $SQLwebsite $SQLwebsiteType $SQLagent AND show_status=1";
            // pre($sql_dataR);
    ...
    public function capacity_download()
    {
        ...
        // $SQLFilterDate = "(`depart_at`< '$DateFrom 09:00:00' AND return_at>='$DateFrom 09:00:00')  AND status='1'  AND show_status=1 ";
