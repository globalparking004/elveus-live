    ...
    if ($csvFile !== FALSE) {
        ...
        while (($dataa = fgetcsv($csvFile)) !== FALSE) {
            ...
            if (in_array($check1, $array)) {
                ...
                if (!empty($Reference) and isset($Reference)) {
                    ...
                    $sql = "INSERT INTO tbl_booking (
                        ...
                        return_at, carReg, status, booked_at,
                         firstName, operator_id, booking_type,airport,RetTerminal,source
                    ) VALUES (
                        '$Reference', '$id', '$OrderPrice', '$DropOffDateTime',
                        '$ReturnDateTime', '$CarReg', '$Status', '$OrderDate',
                        '$CustomerFullName', '$operator_id', 'Online','DUB','$Flight Number','Holiday Extras'
                    )";
                    print_r($sql);
                ...
                $Flight_Number = $conn->real_escape_string($data[16]);
                ...
                if (!empty($Reference) and isset($Reference)) {
                    ...
                    '$Reference', '$id', '$OrderPrice', '$DropOffDateTime',
                    '$ReturnDateTime', '$CarReg', '$Status', '$OrderDate',
                     '$CustomerFullName', '$operator_id', 'Online','DUB','$Flight_Number','Holiday Extras'
                )";
            ...
            if (!empty($Reference) and isset($Reference)) {
                ...
                return_at, carReg, status, booked_at,
                carMake,
                carModel, carColour, firstName, operator_id, booking_type,airport,source
            ) VALUES (
                '$Reference', '$id', '$OrderPrice', '$DropOffDateTime',
                '$ReturnDateTime', '$CarReg', '$Status', '$OrderDate',
                '$Make',
                '$Model', '$Colour', '$CustomerFullName', '$operator_id', 'Online','DUB','Holiday Extras'
            )";
...
$csvFiles = glob($csvDirectory . '*.txt');
// print_r($csvFiles);
if (empty($csvFiles[0])) {
    echo "no file Found";
    exit;
...
foreach ($csvFiles as $csvFilePath) {
    // Extract file name
    $csvFileName = basename($csvFilePath, '.txt');
