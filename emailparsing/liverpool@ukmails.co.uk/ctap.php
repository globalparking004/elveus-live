...
// Directory containing CSV files ,'/var/www/html/email_files/liverpool@ukmails.co.uk/CTAPBookingsbookings@comparetheairportparking.com/'
$array = ['/var/www/html/email_files/liverpool@ukmails.co.uk/bookings@comparetheairportparking.com/','/var/www/html/email_files/liverpool@ukmails.co.uk/CompareTheParkingnoreply@comparetheparking.co.uk/'];
...
function uploadAndInsertCSV($csvFilePath, $tableName, $conn) {
    ...
    if ($csvFile !== FALSE) {
        ...
        while (($data = fgetcsv($csvFile)) !== FALSE) {
            ...
            $CarMake = $conn->real_escape_string($data[14]);
           ...
           if($ProductName)
           {
            ...
            if ($producr_code) {
                ...
                $sql = "INSERT INTO tbl_booking (
                    ...
                    '$Model', '$Colour', '$ContactNumber', '', '$CustomerFullName', '$operator_id','Online','LPL','CTAP'
                )";
                print_r($sql);die;
