...
$csvDirectory = '/var/www/html/email_files/stn@ukmails.co.uk/YourTravelExtrasbooking@yourtravelextras.com/';
...
function uploadAndInsertCSV($csvFilePath, $tableName, $conn) {
    ...
    if ($csvFile !== FALSE) {
        ...
        while (($data = fgetcsv($csvFile)) !== FALSE) {
            ...
            // $ProductName = $conn->real_escape_string($data[1]); 
            $ProductID = $conn->real_escape_string($data[2]);
            ...
            $OrderPrice = $conn->real_escape_string($data[17]);
            // $NoOfPassengers = $conn->real_escape_string($data[16]);
            // $SupplierCost = $conn->real_escape_string($data[18]);
           ...
           if(!empty($Reference) and isset($Reference))
           {
            ...
            $sql = "INSERT INTO tbl_booking (
                ...
                carModel, carColour, contactNumber, firstName,operator_id,booking_type,airport,source
            ) VALUES (
                ...
                '$Model', '$Colour', '$ContactNumber', '$CustomerFullName', '$operator_id','Online','STN','YTE'
            )";
