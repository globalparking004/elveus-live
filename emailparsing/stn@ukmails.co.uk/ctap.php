...
$array = ['/var/www/html/email_files/stn@ukmails.co.uk/bookings@comparetheairportparking.com/','/var/www/html/email_files/stn@ukmails.co.uk/CTAPBookingsbookings@comparetheairportparking.com/'];
...
function uploadAndInsertCSV($csvFilePath, $tableName, $conn) {
    ...
    if ($csvFile !== FALSE) {
        ...
        while (($data = fgetcsv($csvFile)) !== FALSE) {
            ...
            $CustomerFristName = $conn->real_escape_string($data[3]);
            ...
            $OrderDate = $conn->real_escape_string($data[11]);

            $OrderDate =date("Y-m-d H:i:s",strtotime($OrderDate));
            $CustomerFullName = $CustomerFristName.' '.$CustomerLastName;
            ...
            // $DropOffDateTime= date('Y-m-d H:i:s',strtotime($DepartureDate.' '.$DropTime));
            // $ReturnDateTime= date('Y-m-d H:i:s',strtotime($ReturnDate.' '.$ReturnTime));
            ...
            if ($producr_code) {
                ...
                $sql = "INSERT INTO tbl_booking (
                    ...
                    '$Model', '$Colour', '$ContactNumber', '$NoOfPassengers', '$CustomerFullName', '$operator_id','Online','STN','CTAP'
                )";
                print_r($sql);
