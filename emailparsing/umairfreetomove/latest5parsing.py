...
FOLDER_PATH = r'/var/www/html/email_files/bhx/freeeeTomove/'
IMAGES_PATH = os.path.join(FOLDER_PATH, 'images')
email_user = 'bhx@ukmails.co.uk'
...
def extract_details(email_body):
    ...
    # Extract vehicle
    vehicle_match = re.search(r'Vehicle\s*:\s*(.+?)\s*-\s*(\S+)', email_body)
    if vehicle_match:
        details['Vehicle'] = f"{vehicle_match.group(1)} ({vehicle_match.group(2)})"
    else:
        details['Vehicle'] = 'N/A'
