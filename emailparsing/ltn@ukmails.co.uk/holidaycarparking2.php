...
$array = ['/var/www/html/email_files/ltn/HolidaysCarParkingno-reply@holidayscarparking.uk/'];
...
function uploadAndInsertCSV($csvFilePath, $tableName, $conn) {
    ...
    if ($csvFile !== FALSE) {
        ...
        while (($data = fgetcsv($csvFile)) !== FALSE) {
            ...
            $Reference = $conn->real_escape_string($data[3]);
            $Status = $conn->real_escape_string($data[4]);
            $CustomerFullName = $conn->real_escape_string($data[5]);
            $DropOffDateTime = $conn->real_escape_string($data[6]);
            $ReturnDateTime = $conn->real_escape_string($data[7]);
            $DepartureTerminal = $conn->real_escape_string($data[8]);
            $ReturnTerminal = $conn->real_escape_string($data[9]);
            $DepartureFlightNumber = $conn->real_escape_string($data[10]);
            $ReturnFlightNumber = $conn->real_escape_string($data[11]);
            $CarReg = $conn->real_escape_string($data[12]);
            $Make = $conn->real_escape_string($data[13]);
            $Model = $conn->real_escape_string($data[14]);
            $Colour = $conn->real_escape_string($data[15]);
            $NoOfPassengers = $conn->real_escape_string($data[19]);
            $ContactNumber = $conn->real_escape_string($data[20]);
            $OrderPrice = $conn->real_escape_string($data[21]);

            // $SupplierCost = $conn->real_escape_string($data[21]);
            ...
            // || strpos("booked", $Status) !== false
            if (strpos("completed", $Status) !== false || strpos("booked", $Status) !== false) {
                $Status=1;
           ...
           if($ProductName)
           {
                $sql_data = "select id,product_code,operator_id from tbl_products where name='$ProductName' AND parent='LTN' ";
                ...
                if ($row = $sql_data_result->fetch_assoc()) {
                    ...
                    echo "No product found ".$ProductName;
                    exit;
                }

            }else{
                echo "no product match ".$ProductName;
                exit;
            }
           ...
           if(!empty($Reference) and isset($Reference)){
            ...
            $sql = "INSERT INTO tbl_booking (
                ...
                '$Model', '$Colour', '$ContactNumber', '$NoOfPassengers', '$CustomerFullName', '$operator_id','Online','LTN','HCP'
            )";
            print_r($sql);
...
foreach($array as $arr)
{
    ...
    foreach ($csvFiles as $csvFilePath) {
        ...
        echo'csvFile:';print_r($csvFileName);
